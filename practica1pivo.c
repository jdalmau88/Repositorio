...
/*---------------------------------IMPORTANTE MODIFICAR EL NOMBRE ARCHIVO DEPENDIENDO DEL EJERCICIO QUE QUEREMOS REALIZAR----------------------------------------*/
	 entrada = fopen ( "ejerciciopedo.dad","r" ); /* (1) */
	 if ( entrada == NULL ){
...
int gauss(double **A, double **G, int n ,int m, double tol){
	  int i,j,s,w,l=0,inversa=1,p,io;  
	...
	if (inversa==0 || m!=n){
		for (s= 0; s < n-1; s++){ 
		   /* for (io=0; io < n-1; io++){ 
			...
			for (i = s; i < n-1; i++){ 
				 printf("\ndfgds %e   sdfgsdf ",A[s][s]);
				 printf(" fdgdsfg %e   dfgsdfg \n",A[i+1][s]);
			  if(A[s][s]<A[i+1][s]){	
