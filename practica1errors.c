...
int gauss(double **A, double **G, int n ,int m, double tol){
	  ...
	  double *B,**CopiaA,division,*x,**h,**u,**Gaussinicial,*copiab;
	...
	Gaussinicial= (double  **)malloc( n*sizeof(double *) );
	if ( Gaussinicial== NULL ){
		printf("No hi ha prou memoria");
		exit(1);
	}
	for (i = 0; i < n; i++){
		Gaussinicial[i] = (double  *)malloc( n*sizeof(double ) );
		if ( CopiaA[i] == NULL ){
			printf("No hi ha prou memoria");
			exit(2);
		}
	...
	if (inversa==0 || m!=n){
		...
		for (j = 0; j < m; j++){
			...
			l=resoltrisup(n,CopiaA,B,x,tol,Gaussinicial,copiab);
		}
		return l;
	}
	else{
...
int resoltrisup(int n,double **a,double *u,double *x,double tol,double *Gaussinicial,double *copiab){
	...
	copiau = (double  *)malloc( n*sizeof(double ) );
	...
	errors(Gaussinicial,copiau,x,n,copiab);
	return 0;
}
...
int errors(double **A,double *B,double *X,int n, double *copiab){
  double *v,caca;
  int j,i,valor;
  v = (double  *)malloc( n*sizeof(double ) );
  printf("\nLa matriu A = \n");
